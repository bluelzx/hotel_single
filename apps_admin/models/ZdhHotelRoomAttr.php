...
class ZdhHotelRoomAttr extends ActiveRecord {

    public static function tableName() {
        return 'zdh_hotel_room_attr';
    }

    public function saveAttr($_params = []) {
        $Conn = Yii::$app->db;
        $Result = $Conn->createCommand()->insert('zdh_hotel_room_attr',[
    ...
    public function updateAttr($_params = []) {
        ...
        if(isset($_params['id']) && !empty($_params['id'])) {
            $Res = $Conn->createCommand()->update('zdh_hotel_room_attr', [
                'attr_value'=>$_params['attr_value']],
                [
                    "id" => $_params['id'],
                    "room_id" => $_params['room_id'],
