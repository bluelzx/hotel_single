...
?>
<form class="form-horizontal" method="post" action="<?php echo Url::to(['room-type/add']); ?>" enctype="multipart/form-data">
    ...
    seajs.use(['$','sel'],function($,sel){
        ...
        function ajaxSubmit(_param1, _param2,sel) {
            if(1 == _param1 || 1 == _param2) {
                $.ajax({
                    url : "<?php echo Url::to(['room-type/add']); ?>",
                    type: "GET",
                    dataType:"json",
                    success: function(data) {
                        ...
                        for(var i = 0; i < data.length; i++) {
                            //手动填写
                            if(0 == data[i]['attr_type']) {
                                if(!data[i]['attr_suffix_text']) {
                                    $('#clone').clone().attr('id','clone'+i).show().appendTo('#room_detail').find('#btn').hide().parent().find('input').attr({id:"attr"+i,name:data[i]['id']}).val(data[i]['attr_option_values']).parent().find('#room_acr').html(data[i]['attr_name']);
                                } else {
                                    //$('#clone').clone().show().appendTo('#room_detail');
                                    $('#clone').clone().attr('id','clone'+i).show().appendTo('#room_detail').find('input').attr({id:"attr"+i,name:data[i]['id']}).val(data[i]['attr_option_values']).parent().find('#room_acr').html(data[i]['attr_name']).parent().parent().find('.suffix').html(data[i]['attr_suffix_text']);
                                }
                            }

                            //单选
                            if(1 == data[i]['attr_type']) {
                                $('#cradio').clone().attr('id','clone'+i).show().appendTo('#room_detail').find('#room_rad').html(data[i]['attr_name']).attr('id','room_rad'+i);
                                var arr = data[i]['attr_option_values'].split('\n');
                                for(var j = 0; j < arr.length; j++) {
                                    $('#rad').clone().attr('id','rad' + i + j).show().appendTo('#room_rad'+i).find('input').attr({name:data[i]['id'],id:'Radio1_toshop' + j,value:arr[j]}).parent().find('span').html(arr[j]);
                                }
                            }

                            //多选
                            if(2 == data[i]['attr_type']) {
                                $('#cradio').clone().attr('id','clone'+i).show().appendTo('#room_detail').find('#room_rad').html(data[i]['attr_name']).attr('id','room_rad'+i);
                                var arr = data[i]['attr_option_values'].split('\n');
                                for(var j = 0; j < arr.length; j++) {
                                    $('#rad').clone().attr('id','rad' + i + j).show().appendTo('#room_rad'+i).find('input').attr({name:data[i]['id']+"[]",id:'Radio1_toshop' + j,value:arr[j],type:'checkbox'}).parent().find('span').html(arr[j]);
                                }
                            }

                            //下拉
                            if(3 == data[i]['attr_type']) {
                                $('#sel').clone().attr('id','clone'+i).show().appendTo('#room_detail').find('#la').html(data[i]['attr_name']).parent().find('select').attr({id:'sel' + i,name:data[i]['id']});
                                var arr = data[i]['attr_option_values'].split('\n');
                                for(var j = 0; j < arr.length; j++) {
                                    $('#sel' + i).append('<option value="'+arr[j]+'">'+arr[j]+'</option>');
                                }
                            }
                        }
                       // var arr = data[0]['attr_option_values'].split('\n');
                        //console.log(arr[0]);
                    }
                });//end of ajax
            }
