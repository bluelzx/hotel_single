...
<div class="page-content">
    <form class="form-horizontal" method="post" action="<?php echo Url::to(['category/edit-attr','id' => $id,'type_id' => $type_id]); ?>">
        <!-- /.page-header -->
        <!-- PAGE CONTENT BEGINS -->
        <!--酒店设置--[start]-->
        <div class="row">
...
<script type="text/javascript">
    ...
    function ajaxSubmit(_param1, _param2) {
        if(1 == _param1 || 1 == _param2) {
            $.ajax({
                url : "<?php echo Url::to(['category/edit-attr','id' => $id]); ?>",
                type: "GET",
                dataType:"json",
                success: function(data) {
                    $('#attr_name').val(data['attr_name']);
                    $('#attr_value').val(data['attr_option_values']);
                    $("input[name=type]:eq("+data['attr_type']+")").attr('checked','checked');
                    $('#attr_suffix_text').val(data['attr_suffix_text']);
                }
            });//end of ajax
        }
    }
