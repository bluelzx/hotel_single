...
?>
<form class="form-horizontal" method="post" action="<?php echo Url::to(['room/add']); ?>">
                    ...
                    success: function(data) {
                        ...
                        for(var i = 0; i < data['room_detail'].length; i++) {
                            ...
                            if(3 == data['room_detail'][i]['attr_type']) {
                                //$('#sel').clone().attr('id',data['room_detail'][i]['attr_name']).show().appendTo('#room_detail').find('#la').html(data['room_detail'][i]['attr_name']).parent().find('select').attr({id:data['room_detail'][i]['id'],name:data['room_detail'][i]['id']});
