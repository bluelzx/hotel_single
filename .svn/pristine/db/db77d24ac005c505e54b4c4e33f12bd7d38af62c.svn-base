            ...
            <div class="row">
                <div class="tabbable">
                    <ul class="nav nav-tabs" id="myTab">
                        ...
                        <li>
                            <a  href="<?php echo Url::to(['room-price/add']) ?>">
    ...
    seajs.use(['admin-common','$','dataTables',"dataTables-bootstrap"],function(admin,$){
        var oTable = $('#sample-table').dataTable({
            ...
            "aoColumns" : [ //列操作
                ...
                {"mData" : "oprice",bSortable:false,"sWidth" : "15%"},
                {"mData" : "cprice",bSortable:false,"sWidth" : "15%"},
                {
                    "mData" : "status",
                    "sClass" : "center",
                    bSortable:false,
                ...
                },
                {
                    ...
                    "mRender" : function (data,type,full) {
                        var editurl = "<?php echo Url::to(['room-price/update']) ?>&id=" + data;
                        var delturl = "<?php echo Url::to(['room-price/del']) ?>&id=" + data;
            ...
            "sAjaxSource" : "<?php echo Url::to(['room-price/index']) ?>",
            "sServerMethod" : "POST",
            //"bProcessing": true,
        ...
        $(document).on('click', 'input.ace', function () {
            ...
            if(ob.is(':checked')) {
                c = 1;
                _url = "<?php echo Url::to(['room-price/show']); ?>&id="+id+"&c="+c;
                console.log("<?php echo Url::to(['room-price/show']); ?>&id="+id+"&c="+c);
                $.ajax({
                    url : _url,
                    success : function (data) {
                        if ( data.status > 0 ) {
